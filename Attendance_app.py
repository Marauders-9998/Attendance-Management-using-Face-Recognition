...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(self, text="Go to Student Portal", width=30,
                            command=lambda: self.doWork("StudentPanelPage", self.tv_class.get(), self.tv_username.get(),
                                                        self.tv_pass.get()))
        button2 = tk.Button(self, text="Go to Manager Portal", width=30,
                            command=lambda: self.doWork("ManagerPanelPage", self.tv_class.get(), self.tv_username.get(),
                                                        self.tv_pass.get()))
        button1.pack()
        button2.pack()
...
class CreateNewBatchPage(tk.Frame):
    ...
    def validate_number_field(self, action, index, value_if_allowed, prior_value, text, validation_type, trigger_type,
                              widget_name):
    ...
    def doWork(self):
        ...
        try:  # If current month exists
            ...
            ws.cell(row=row_here, column=sum_cell_column,
                    value="=SUM(" + first_date_column + str(row_here) + ":" + last_date_column + str(
                        row_here) + ")").fill = fill_total
            wb.save(atten_reg)
        ...
        while i > 0:
            ...
            img_path = os.path.join(os.getcwd(), 'images', current_class, "s" + str(roll_no).zfill(2),
